...
ASR_DIR="$ROOT/modules/asr"
CONFIG_DIR="$ROOT/config"

say "创建讯飞ASR模块..."

# 备份原文件
if [ -f "$ASR_DIR/asr_module.py" ]; then
    cp "$ASR_DIR/asr_module.py" "$ASR_DIR/asr_module.py.bak_$(date +%Y%m%d_%H%M%S)"
    say "备份原文件..."
fi

# 创建讯飞ASR模块
cat > "$ASR_DIR/xunfei_asr.py" <<'PYCODE'
#!/usr/bin/env python3
"""
讯飞实时语音识别模块
"""
...
# 创建配置文件模板
cat > "$CONFIG_DIR/xunfei_asr.json" <<'JSON'
{
    "app_id": "YOUR_APP_ID",
...
# Python语法检查
if python3 -m py_compile "$ASR_DIR/xunfei_asr.py" 2>/dev/null; then
    say "Python语法检查: OK"
else
    say "Python语法检查: 失败"
    exit 1
...
say "讯飞ASR模块创建完成"
say "请配置讯飞API信息: $CONFIG_DIR/xunfei_asr.json"
say ""
say "配置步骤："
say "1. 编辑 $CONFIG_DIR/xunfei_asr.json"
say "2. 填入讯飞控制台的 app_id, api_key, api_secret"
say "3. 运行: python3 $ASR_DIR/xunfei_asr.py"
