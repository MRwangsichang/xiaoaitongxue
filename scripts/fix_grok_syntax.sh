...
GROK_CLIENT="${ROOT}/modules/llm/grok_client.py"

say "=== 修复grok_client.py语法错误 ==="

# 备份
cp -p "${GROK_CLIENT}" "${GROK_CLIENT}.broken"

# 重新创建完整的grok_client.py
cat > "${GROK_CLIENT}" <<'PYCODE'
"""Grok API客户端"""
import os
import json
import asyncio
...
# 验证语法
if python3 -m py_compile "${GROK_CLIENT}" 2>/dev/null; then
    say "✓ 语法错误已修复"
else
    say "✗ 仍有语法错误"
    python3 -m py_compile "${GROK_CLIENT}"
    exit 1
fi

say ""
say "重启Rules测试："
echo "  【终端2】bash scripts/start_rules.sh"
