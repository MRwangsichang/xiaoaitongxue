...
echo "=== CREATING CONFIG LOADER ==="

if [ "$DRY_RUN" = "1" ]; then
  say "DRY-RUN: Would create core/config_loader.py"
  say "DRY-RUN: Would create config/app.yml (sample)"
  say "DRY-RUN: Would create tests/test_config.py"
  exit 0
fi

# 1. 创建配置加载器
say "Creating core/config_loader.py..."
cat > "$ROOT/core/config_loader.py" <<'PYCODE'
"""
...
# 2. 创建示例配置文件
say "Creating config/app.yml (sample)..."
cat > "$ROOT/config/app.yml" <<'YAMLCODE'
# Smart Assistant 核心配置
# 版本: v2.1
...
# 3. 创建测试用例
say "Creating tests/test_config.py..."
cat > "$ROOT/tests/test_config.py" <<'PYTEST'
"""
配置加载器测试
"""
import sys
