...
TASKS=("正脸" "正脸" "微笑" "表情1" "表情2")

say "准备开始正常光照采集，请确认已开灯"
sleep 3

TOTAL=50
for BATCH in {6..10}; do
    TASK="${TASKS[$((BATCH-6))]}"
    say ""
...
PYEOF
    ...
    if [ $BATCH -lt 10 ]; then
        say ">>> 停顿5秒，请调整姿势 - 下一组: ${TASKS[$((BATCH-5))]}"
        for s in {5..1}; do
            printf "\r    倒计时: %d秒..." $s
            sleep 1
        done
...
say ""
say "WinSCP查看: /home/MRwang/smart_assistant/data/faces/王总/processed/"
