...
class TTSModule:
    ...
    async def start(self):
        ...
        # 4. 订阅TTS请求
        self._listen_task = asyncio.create_task(self.bus.start_listening({
            "sa/tts/say": self._handle_tts_request
        }))
        
        # 5. 启动播放队列处理
        self.running = True
        self.logger.info("DEBUG: 准备启动播放循环")
        self._player_task = asyncio.create_task(self._player_loop())
        self.logger.info("DEBUG: 播放循环已启动")
    ...
    async def _player_loop(self):
        self.logger.info("DEBUG: _player_loop 开始执行")
        """播放队列处理循环"""
        while self.running:
            try:
                # 等待队列有数据
