...
class ASRModule:
    ...
    async def start(self):
        ...
        try:
            ...
            # Initialize MQTT bus
            self.bus = EventBus(module_name="asr")
    ...
    async def stop(self):
        ...
        self.logger.info("=== _init_provider 开始执行 ===")
        if self.bus:
    ...
    async def _init_provider(self):
        ...
        if provider_type == "mock":
            ...
            if missing:
                ...
                from modules.asr.mock_provider import MockProvider
                self.provider = MockProvider(self.config, self.logger)
                self.logger.info("开始初始化 CloudProvider...")
            else:
                self.logger.info("CloudProvider 初始化完成")
    ...
    async def _handle_command(self, envelope):
        ...
        if hasattr(envelope, "event_type"):
            self.logger.info(f"DEBUG event_type: {envelope.event_type}")
        payload = envelope.payload if hasattr(envelope, "payload") else envelope
        event_type = envelope.__dict__.get('type', '') or getattr(envelope, 'type', '')
        # 重构topic以兼容旧逻辑
        topic = f"sa/asr/cmd/{event_type.split('.')[-1] if '.' in event_type else 'unknown'}"
        cmd = topic.split("/")[-1]
    ...
    async def _handle_start(self):
        self.logger.info("=== _handle_start被调用 ===")
        self.logger.info(f"DEBUG: self.provider = {self.provider}")
        """Start ASR session"""
    ...
    async def _on_recognition_result(self, result: dict):
        ...
        await self.bus.publish("sa/asr/text", "text", payload)
    ...
    async def _publish_error(self, error_type: str, message: str):
        """Publish error event"""
        if self.bus:
            await self.bus.publish("sa/asr/error", "error", {
                "type": error_type,
                "message": message,
                "ts": time.time()
            })
